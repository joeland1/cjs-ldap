GPP_DEFAULT="-static -fPIC 
    -I/code/ldap-async-addon/node_modules/node-addon-api 
    -I/code/ldap-lib/include 
    -I/usr/include/node 
    -std=c++23 -mno-outline-atomics 
    -c"
GPP_FLAGS="${GPP_FLAGS:-${GPP_DEFAULT}}"

LDAP_LIB_DEFAULT="/code/ldap-lib/lib"
LDAP_LIB="${LDAP_LIB:-${LDAP_LIB_DEFAULT}}"

g++ $GPP_FLAGS -o addon.o addon.cpp || exit 1

g++ $GPP_FLAGS -o client.o client.cpp || exit 1

g++ $GPP_FLAGS -o async-bind-ldap.o async-bind-ldap.cpp -lldap -llber || exit 1

g++ $GPP_FLAGS -o async-ldap-search.o async-ldap-search.cpp -lldap -llber || exit 1

g++ $GPP_FLAGS -o async-ldap-close.o async-ldap-close.cpp -lldap -llber || exit 1

g++ $GPP_FLAGS -o search_values.o search_values.cpp || exit 1

g++ $GPP_FLAGS -o ldap-result.o ldap-result.cpp || exit 1

g++ $GPP_FLAGS -o ldap-result-obj.o ldap-result-obj.cpp || exit 1

#g++ $GPP_FLAGS -o generic-async.o generic-async.cpp || exit 1

g++ $GPP_FLAGS -o tls_validation_settings.o tls_validation_settings.cpp || exit 1
